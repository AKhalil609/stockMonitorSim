...
describe('StockSubscriber', () => {
  ...
  beforeEach(() => {
    ...
    (stockService.getLoading as jest.Mock).mockReturnValue(loading$);
  });
  ...
  it('should enable subscribe button when ISIN is valid and connection is active', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Enter ISIN'), {
      target: { value: 'validISIN' },
    });
  ...
  it('should display error message when connection is lost', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Connection lost. Prices may not be accurate.')
      ).toBeInTheDocument();
    });
  });
  ...
  it('should call unsubscribeFromStocks when unsubscribe button is clicked', async () => {
    ...
    fireEvent.change(screen.getByRole('textbox'), {
      target: { value: 'DE000BASF111' },
    });
    fireEvent.click(screen.getByLabelText('unsubscribe'));
    await waitFor(() => {
      expect(stockService.unsubscribeFromStocks).toHaveBeenCalledWith([
        'DE000BASF111',
      ]);
    });
  });
